...
static int
fail(int code, const char *format, ...)
{
	va_list args;
...
struct address_range
{
	...
	address_range(uint32_t from, uint32_t to, type type)
...
const address_ranges rp2040_address_ranges_flash{
	...
	address_range(MAIN_RAM_START, MAIN_RAM_END,
				  address_range::type::NO_CONTENTS)
};
...
const address_ranges rp2040_address_ranges_ram{
	...
	address_range(0x00000000u, 0x00004000u,
				  address_range::type::IGNORE)	  // for now we ignore the
												  // bootrom if present
};
...
static int
read_and_check_elf32_header(FILE *in, elf32_header &eh_out)
{
...
int
check_address_range(const address_ranges &valid_ranges, uint32_t addr,
					uint32_t vaddr, uint32_t size, bool uninitialized,
					address_range &ar)
{
	for (const auto &range : valid_ranges) {
		if (range.from <= addr && range.to >= addr + size) {
			if (range.type == address_range::type::NO_CONTENTS &&
				!uninitialized) {
				return fail(
					ERROR_INCOMPATIBLE,
					"ELF contains memory contents for uninitialized memory");
			}
			ar = range;
			if (verbose) {
	...
	return fail(ERROR_INCOMPATIBLE,
				"Memory segment %08x->%08x is outside of valid address range "
				"for device",
				addr, addr + size);
}

int
read_and_check_elf32_ph_entries(
	FILE *in, const elf32_header &eh, const address_ranges &valid_ranges,
	std::map<uint32_t, std::vector<page_fragment>> &pages)
{
	...
	if (eh.ph_num) {
		...
		if (eh.ph_num !=
			fread(&entries[0], sizeof(struct elf32_ph_entry), eh.ph_num, in)) {
		...
		for (uint i = 0; i < eh.ph_num; i++) {
			...
			if (entry.type == PT_LOAD && entry.memsz) {
				...
				if (mapped_size) {
					rc = check_address_range(valid_ranges, entry.paddr,
											 entry.vaddr, mapped_size, false,
											 ar);
					if (rc)
						return rc;
					// we don't download uninitialized, generally it is BSS and
					// should be zero-ed by crt0.S, or it may be COPY areas
					// which are undefined
					...
					while (remaining) {
						...
						auto &fragments =
							pages[addr - off];	  // list of fragments
						// note if filesz is zero, we want zero init which is
						// handled because the statement above creates an empty
						// page fragment list check overlap with any existing
						// fragments
						for (const auto &fragment : fragments) {
							if ((off < fragment.page_offset + fragment.bytes) !=
								((off + len) <= fragment.page_offset)) {
								fail(ERROR_FORMAT,
									 "In memory segments overlap");
							}
						}
						fragments.push_back(
				...
				if (entry.memsz > entry.filez) {
					...
					rc = check_address_range(
						valid_ranges, entry.paddr + entry.filez,
						entry.vaddr + entry.filez, entry.memsz - entry.filez,
						true, ar);
					if (rc)
						return rc;
				}
...
int
realize_page(FILE *in, const std::vector<page_fragment> &fragments,
			 uint8_t *buf, uint buf_len)
{
	assert(buf_len >= PAGE_SIZE);
	for (auto &frag : fragments) {
		assert(frag.page_offset >= 0 && frag.page_offset < PAGE_SIZE &&
...
static bool
is_address_valid(const address_ranges &valid_ranges, uint32_t addr)
{
	for (const auto &range : valid_ranges) {
...
static bool
is_address_initialized(const address_ranges &valid_ranges, uint32_t addr)
{
	for (const auto &range : valid_ranges) {
		if (range.from <= addr && range.to > addr) {
...
static bool
is_address_mapped(const std::map<uint32_t, std::vector<page_fragment>> &pages,
				  uint32_t addr)
{
...
elf2uf2(FILE *in, FILE *out)
{
	...
	if (!rc) {
		...
		valid_ranges =
			ram_style ? rp2040_address_ranges_ram : rp2040_address_ranges_flash;
	...
	if (ram_style) {
		...
		if (eh.entry != expected_ep) {
			return fail(ERROR_INCOMPATIBLE,
						"A RAM binary should have an entry point at the "
						"beginning: %08x (not %08x)\n",
						expected_ep, eh.entry);
		}
		static_assert(0 == (MAIN_RAM_START & (PAGE_SIZE - 1)), "");
		// currently don't require this as entry point is now at the start, we
		// don't know where reset vector is
#if 0
	...
	for (auto &page_entry : pages) {
		...
		rc =
			realize_page(in, page_entry.second, block.data, sizeof(block.data));
		if (rc)
			return rc;
